...
if($_SESSION['usn']) {
    ...
    $usn=$_SESSION['usn'];
    $q="SELECT * FROM shakuntalamma where usn='$usn'";
    $r=mysqli_query($db,$q);
    ...
    if(isset($_POST['submit'])) {
        ...
        $s4=$_FILES['signature']['size'];
        if($s1<100000 and $s2<100000 and $s3<100000 and $s4<100000) {
            $puc=$_SESSION['usn'].$_FILES['puc']['name'];
            $destination1='../uploads/shakuntalamma/'.$puc;
            $extension = pathinfo($puc, PATHINFO_BASENAME);
            $file1 = $_FILES['puc']['tmp_name'];

            $markscard1=$_SESSION['usn'].$_FILES['markscard1']['name'];
            $destination2='../uploads/shakuntalamma/'.$markscard1;
            ...
            $destination3='../uploads/shakuntalamma/'.$markscard2;
            ...
            $destination4='../uploads/shakuntalamma/'.$signature;
            $extension = pathinfo($signature, PATHINFO_BASENAME);
            $file4 = $_FILES['signature']['tmp_name'];

            if (move_uploaded_file($file1, $destination1) and move_uploaded_file($file2, $destination2) and move_uploaded_file($file3, $destination3) and move_uploaded_file($file4, $destination4)) {
                $query="INSERT INTO shakuntalamma(usn,puc,markscard1,markscard2,signature) VALUES('$usn','$puc','$markscard1','$markscard2','$signature')";
                if (!mysqli_query($db, $query)) {
                    echo "Error".mysqli_error($db);
                } else {
                    $q2="SELECT email FROM studentregister WHERE usn='$usn'";
                    $q3="SELECT app_no FROM shakuntalamma WHERE usn='$usn'";
                    ...
                    <body>
                        ...
                        <div style='text-align: center; padding-left: 2rem; padding-right: 2rem;'>
                            ...
                            <h2>Smt Pathre Shakuntalamma Scholarship</h2>
...
<head>
    ...
    <title>Smt Pathre Shakuntalamma Scholarship</title>
</head>
<body>
...
<div class="main">
<h1>Smt Pathre Shakuntalamma Scholarship</h1>
