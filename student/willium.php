...
if($_SESSION['usn']) {
    ...
    $usn=$_SESSION['usn'];
    $q="SELECT * FROM willium where usn='$usn'";
    $r=mysqli_query($db,$q);
    ...
    if(isset($_POST['submit'])) {
        $usn=$_SESSION['usn'];
        $s1=$_FILES['puc']['size'];
        $s2=$_FILES['caste']['size'];
        $s3=$_FILES['markscard1']['size'];
        $s4=$_FILES['markscard2']['size'];
        ...
        if($s1<100000 and $s2<100000 and $s3<100000 and $s4<100000 and $s5<100000) {
            ...
            $destination1='../uploads/willium/'.$puc;
            $extension = pathinfo($puc, PATHINFO_BASENAME);
            $file1 = $_FILES['puc']['tmp_name'];

            $caste=$_SESSION['usn'].$_FILES['caste']['name'];
            $destination2='../uploads/willium/'.$caste;
            $extension = pathinfo($caste, PATHINFO_BASENAME);
            $file2 = $_FILES['caste']['tmp_name'];

            $markscard1=$_SESSION['usn'].$_FILES['markscard1']['name'];
            $destination3='../uploads/willium/'.$markscard1;
            $extension = pathinfo($markscard1, PATHINFO_BASENAME);
            $file3 = $_FILES['markscard1']['tmp_name'];

            $markscard2=$_SESSION['usn'].$_FILES['markscard2']['name'];
            $destination4='../uploads/willium/'.$markscard2;
            $extension = pathinfo($markscard2, PATHINFO_BASENAME);
            $file4 = $_FILES['markscard2']['tmp_name'];

            $signature=$_SESSION['usn'].$_FILES['signature']['name'];
            $destination5='../uploads/willium/'.$signature;
            ...
            if (move_uploaded_file($file1, $destination1) and move_uploaded_file($file2, $destination2) and move_uploaded_file($file3, $destination3) and move_uploaded_file($file4, $destination4) and move_uploaded_file($file5, $destination5)) {
                $query="INSERT INTO willium(usn,puc,caste,markscard1,markscard2,signature) VALUES('$usn','$puc','$caste','$markscard1','$markscard2','$signature')";
                if (!mysqli_query($db, $query)) {
                    echo "Error".mysqli_error($db);
                } else {
                    $q2="SELECT email FROM studentregister WHERE usn='$usn'";
                    $q3="SELECT app_no FROM willium WHERE usn='$usn'";
                    ...
                    <body>
                        ...
                        <div style='text-align: center; padding-left: 2rem; padding-right: 2rem;'>
                            ...
                            <h2>Mr Willium Mitchell Scholarship</h2>
...
<head>
    ...
    <title>Mr Willium Mitchell Scholarship</title>
</head>
<body>
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <div class="main">
    <h1>Mr Willium Mitchell Scholarship</h1>
    <form action="#" method="post" enctype="multipart/form-data">
        <table align="center">
            <tbody>
                ...
                <td><input type="file" name="caste" ></td>
                </tr>
                <tr>
