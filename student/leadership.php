...
if($_SESSION['usn']) {
    ...
    $usn=$_SESSION['usn'];
    $q="SELECT * FROM leadership where usn='$usn'";
    $r=mysqli_query($db,$q);
    ...
    if(isset($_POST['submit'])) {
        $usn=$_SESSION['usn'];
        $s1=$_FILES['puc']['size'];
        $s2=$_FILES['document']['size'];
        $s3=$_FILES['signature']['size'];
        if($s1<100000 and $s2<100000 and $s3<100000) {
            $puc=$_SESSION['usn'].$_FILES['puc']['name'];
            $destination1='../uploads/leadership/'.$puc;
            $extension = pathinfo($puc, PATHINFO_BASENAME);
            $file1 = $_FILES['puc']['tmp_name'];

            $document=$_SESSION['usn'].$_FILES['document']['name'];
            $destination2='../uploads/leadership/'.$document;
            $extension = pathinfo($document, PATHINFO_BASENAME);
            $file2 = $_FILES['document']['tmp_name'];

            $signature=$_SESSION['usn'].$_FILES['signature']['name'];
            $destination3='../uploads/leadership/'.$signature;
            ...
            if (move_uploaded_file($file1, $destination1) and move_uploaded_file($file2, $destination2) and move_uploaded_file($file3, $destination3)) {
                $query="INSERT INTO leadership(usn,puc,document,signature) VALUES('$usn','$puc','$document','$signature')";
                if (!mysqli_query($db, $query)) {
                    echo "Error".mysqli_error($db);
                } else {
                    $q2="SELECT email FROM studentregister WHERE usn='$usn'";
                    $q3="SELECT app_no FROM leadership WHERE usn='$usn'";
                    ...
                    <body>
                        ...
                        <div style='text-align: center; padding-left: 2rem; padding-right: 2rem;'>
                            ...
                            <h2>Mr Willium Mitchell Scholarship for demonstrated Leadership</h2>
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <div class="main">
    <h1 class="label">Mr Willium Mitchell Scholarship for demonstrated Leadership.</h1>
    <form action="#" method="post" enctype="multipart/form-data">
        <table align="center">
            <tbody>
                ...
                <tr>
                <td><label>All other documents/certificate partaining to scholarship</label></td>
 ...
 Knowledge and belief.</label><br>
  ...
  <div class="footer-copyright text-center py-3">© 2019 Copyright:
    <a href="#">Akash Hiregoudar</a> |Designed and Developed by Akash Hiregoudar.
  </div>
</footer>
</body>
</html>